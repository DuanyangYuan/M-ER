...
You are an expert, given a programming problem and some corresponding solution steps (not necessarily complete). You need to determine whether the given steps have solved the problem and provided an answer.

You need to distinguish between two cases and provide the corresponding output:

    If the given steps have analyzed the required steps as per the problem's requirements, then directly output: "<end>", without any additional content.

    If the given steps have not yet analyzed the required steps, then directly output: "<continue>", without any additional content.
    Note that if the existing steps do not analyze the answer as required by the problem, it should be considered unresolved.
    Below is the input, please provide the output according to the required format. You do not need to solve the problem.
...
single_reflection_prompt_zh = '''
你是一个专家，给定一个多跳问答问题，我已经使用蒙特卡洛树搜索进行节点搜索，需要你判断当前搜索到的节点是否能够回答这个问题。你需要先判断当前节点是否已经能够回答问题，如果还不能回答，请你基于你的已有知识储备给出针对下一步要检索的三元组中的节点和关系的名称。

你需要区分两种情况给出对应的输出:
1.如果当前节点和关系已经能够回答问题并且给出了答案，那么请直接输出:"<end>"即可，不需要输出其他内容。
2.如果当前节点和关系还不能回答问题，你需要基于你的已有知识给出下一步要检索的三元组中的节点和关系的名称，用英语直接以列表的形式输出节点和关系的名称，不需要输出其他内容。如果输入没有提供任何已有步骤，那么请直接输出:"<end>"即可，不需要输出其他内容。
输出格式要求为:
1. 以扁平化列表输出，格式为["relation_name1", "relation_name2", "node_name3", "relation_name4"...]
2. 以英语形式输出
注意:
1. 禁止解释推理过程
2. 禁止使用中文和特殊符号
3. 不要重复已有步骤中的关系
下面是输入，请你按照要求的输出方式进行输出，不要试图解答多跳问答的答案。

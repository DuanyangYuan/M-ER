...
class BasicDataLoader(object):
    # 这段代码定义了一个数据加载器类，它负责加载、解析和准备知识问答任务所需的数据。
    # 它包括了构建实体和关系映射、加载数据文件、准备数据批次和邻接矩阵等步骤。
    ...
    def _load_file(self, config, data_type="train"):
        data_file = os.path.join(config['data_folder'], data_type + "_simple_mcts_expand_llmsim.json")
        ...
        # 打印最大事实数量，更新数据数量，并创建批次索引数组。
        print('max_facts: ', self.max_facts)
        self.num_data = len(self.data)
        self.batches = np.arange(self.num_data)

    def _load_data(self):
        # 打印转换实体索引的信息，并调用 _build_global2local_entity_maps 方法创建全局到本地实体映射。
        ...
        # 初始化问题ID列表、候选实体数组、知识库邻接矩阵数组和问题邻接矩阵数组。
        self.question_id = []
    ...
    def _prepare_data(self):
        """
        这段代码定义了一个名为 _prepare_data 的方法，主要用于处理输入数据，构建与知识图谱（Knowledge Graph）相关的实体、关系和查询信息。
        这段代码的主要功能是从输入数据中提取实体和关系，并构建一个知识图谱的子图。它还统计了不同类型的查询实体的数量，并将结果输出。
        ...
        """
        # 初始化 next_id 为 0，用于跟踪当前样本的索引。
        # num_query_entity 是一个字典，用于存储每个样本中查询实体的数量。
        next_id = 0
        num_query_entity = {}
        for sample in tqdm(self.data):
            ...
            # 初始化一个集合 tp_set 用于存储当前样本的本地实体，
            # 初始化一个列表 seed_list 用于存储种子实体。
            tp_set = set()
            seed_list = []

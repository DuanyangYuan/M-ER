...
class BaseModel(torch.nn.Module):
    ...
    def calc_h3(self, curr_dist, dist_ans, eps=0.01):
        topk = 3
        greedy_options = torch.topk(curr_dist, k=topk, dim=-1)[1]
        dist_topk = torch.zeros_like(curr_dist).scatter_(1, greedy_options, 1.0)
        dist_ans = (dist_ans > eps).float()
        h3 = torch.sum(dist_topk * dist_ans, dim=-1)
        return (h3 > 0).float()

    def calc_h10(self, curr_dist, dist_ans, eps=0.01):
        topk = 10
        greedy_options = torch.topk(curr_dist, k=topk, dim=-1)[1]
        dist_topk = torch.zeros_like(curr_dist).scatter_(1, greedy_options, 1.0)
        dist_ans = (dist_ans > eps).float()
        h10 = torch.sum(dist_topk * dist_ans, dim=-1)
        return (h10 > 0).float()
    ...
    def get_eval_metric(self, pred_dist, answer_dist):
        with torch.no_grad():
            ...
            h3 = self.calc_h3(curr_dist=pred_dist, dist_ans=answer_dist, eps=VERY_SMALL_NUMBER)
            h10 = self.calc_h10(curr_dist=pred_dist, dist_ans=answer_dist, eps=VERY_SMALL_NUMBER)

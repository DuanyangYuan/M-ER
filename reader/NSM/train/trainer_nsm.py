...
# 是一个用于训练和评估知识问答（KBQA）模型的 Trainer 类。
class Trainer_KBQA(object):
    ...
    def optim_def(self):
        # 过滤出需要梯度的参数，并使用 Adam 优化器。如果衰减率大于 0，则创建一个指数学习率调度器。
    ...
    def evaluate(self, data, test_batch_size=20, mode="teacher", write_info=True):
    ...
    def train(self, start_epoch, end_epoch):
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            st = time.time()
            loss, extras, h1_list_all,h3_list_all,h10_list_all, f1_list_all = self.train_epoch()
            if self.decay_rate > 0:
                self.scheduler.step()
            ...
            self.logger.info("Training h1 : {:.4f},h3 : {:.4f},h10 : {:.4f}, f1 : {:.4f}".format(np.mean(h1_list_all),np.mean(h3_list_all),np.mean(h10_list_all), np.mean(f1_list_all)))
    ...
    def train_epoch(self):
        ...
        for iteration in tqdm(range(num_epoch)):
            ...
            h1_list,h3_list,h10_list, f1_list = tp_list
            h1_list_all.extend(h1_list)
            h3_list_all.extend(h1_list)
            h10_list_all.extend(h1_list)
        ...
        extras = [0, 0]
        return np.mean(losses), extras, h1_list_all,h3_list_all,h10_list_all, f1_list_all
